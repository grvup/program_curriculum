  ...
  workingCurriculums: [
    {
      ...
      version: '1.0',
      batch: ['B.Tech CSE 2021', 'B.Tech CSE 2022'],
      semesters: 8
    },
    {
      name: 'CSE UG Curriculum',
...
const BTechCSEView = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState('info');

  const renderInfo = () => (
    <div className="curriculum-info">
      <h2 className="curriculum-title">B.Tech CSE</h2>
      ...
      <table className="discipline-table">
        ...
        <tbody>
          {CURRICULUM_DATA.workingCurriculums.map((curr, idx) => (
            <tr key={idx}>
              <td><a href="#" className="text-primary">{curr.name}</a></td>
              <td>{curr.version}</td>
              <td>
                {curr.batch.map((b, i) => (
                  <React.Fragment key={i}>
                    <a href="#" className="text-primary">{b}</a>
